 ...
 *  independent value (ID=1), the API_KEY itself.
 *  In general each Movie has multiple Xtra entries, even some with variable sizes.
 *  To guarantee a unique and determined ID, the movieID is concatenated with a particular subID
 *  (2 digits).
 *
 */
...
public class Xtra {
     ...
     */
    public Xtra(int ID, String value) {
        this.ID = ID;
        this.value = value;
    }
     ...
     *          ID is the concatenation of movieID  +  subID (2 digits),  cast back to an Integer.
    ...
    @Ignore public Xtra(int movieID, String ATTRIBUTE, String VALUE ) {
        ...
        this.ID = Integer.parseInt(movieID + subID);
        this.value = VALUE;
    }
    ...
    @Ignore public Xtra( int movieID, String ATTRIBUTE, int i, String VALUE ) {
        switch (ATTRIBUTE) {
            case ("videos"):     this.ID = Integer.parseInt(    String.valueOf(movieID) + (20 + i));   break;
            case ("reviews"):    this.ID = Integer.parseInt(    String.valueOf(movieID) + (40 + i));   break;
        }
        this.value = VALUE;
    }
